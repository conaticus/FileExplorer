...
mod tests_hash_commands {
    ...
    fn test_hash_directory() {
        run_async_test(async {
            ...
            // Test directory hash calculation - should fail with FileOperationError
            let result = gen_hash_and_copy_to_clipboard_impl(
                temp_dir.path().to_str().unwrap().to_string(),
                state.clone()
            ).await;

            assert!(result.is_err(), "Directory hash generation should fail");
    ...
    fn test_hash_method_switching() {
        run_async_test(async {
            ...
            // Test with different hash methods sequentially
            let methods = vec![
                ChecksumMethod::MD5,
    ...
    fn test_sha256_large_known_vector() {
        run_async_test(async {
            ...
            // The input pattern from SHA-3 test vectors
