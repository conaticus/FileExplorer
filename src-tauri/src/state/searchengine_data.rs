...
mod tests_searchengine_state {
    ...
    #[test]
    fn test_chunked_concurrent_operations() {
        let settings_state = Arc::new(Mutex::new(SettingsState::new()));
        let state = Arc::new(SearchEngineState::new(settings_state));
        ...
        let mut test_files = Vec::new();
        for i in 0..1000 {
            let file_path = test_dir.join(format!("chunked_concurrent_test_{}.txt", i));
            let _ = fs::write(&file_path, format!("Chunked concurrent test content {}", i));
            test_files.push(file_path);
        }
        ...
        // Try to search while chunked indexing - should return an error
        let search_result = state.search("file");
        assert!(search_result.is_err());
        assert!(search_result.unwrap_err().contains("indexing"));
